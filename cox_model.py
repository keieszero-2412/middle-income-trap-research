"""
Cox PH Model - Branch: credit-only
Variables: AGEDEP, GE, IND, CREDIT
"""
import pandas as pd
import matplotlib
matplotlib.use('Agg')
...
print("=" * 60)
print("COX PH MODEL - CREDIT-ONLY BRANCH")
print("=" * 60)
...
cph_combined.fit(
    df[['start', 'stop', 'event', 'is_UM', 'AGEDEP', 'GE', 'IND', 'CREDIT']],
    duration_col='stop', event_col='event', entry_col='start'
)
cph_combined.print_summary()
fig, ax = plt.subplots(figsize=(10, 6))
cph_combined.plot(ax=ax)
ax.set_title('Cox PH - Hazard Ratios (Combined LM+UM) [CREDIT branch]', fontsize=14)
plt.tight_layout()
...
cph_lm = CoxPHFitter()
cph_lm.fit(
    df_lm[['start', 'stop', 'event', 'AGEDEP', 'GE', 'IND', 'CREDIT']],
    duration_col='stop', event_col='event', entry_col='start'
)
cph_lm.print_summary()
fig, ax = plt.subplots(figsize=(10, 6))
cph_lm.plot(ax=ax)
ax.set_title('Cox PH - LM to UM [CREDIT branch]', fontsize=14)
plt.tight_layout()
...
cph_um = CoxPHFitter()
cph_um.fit(
    df_um[['start', 'stop', 'event', 'AGEDEP', 'GE', 'IND', 'CREDIT']],
    duration_col='stop', event_col='event', entry_col='start'
)
cph_um.print_summary()
fig, ax = plt.subplots(figsize=(10, 6))
cph_um.plot(ax=ax)
ax.set_title('Cox PH - UM to H [CREDIT branch]', fontsize=14)
plt.tight_layout()
...
ax.set_title('Kaplan-Meier Survival Curves [CREDIT branch]', fontsize=14)
